...
int main(){
    ...
    for(i=0; i<50; i++){ // Linha superior
        gotoxy(i,0);
        printf("%c",219);
        Sleep(5); // Pausa execução por 5 milissegun.
    }
    ...
    for(i=50; i>=0; i--){ // Linha inferior
        gotoxy(i,20);
        printf("%c",219);
        Sleep(05); // Pausa execução por 5 milissegun.
    }
    ...
    while(gameover==0){ // Enquanto o usuário não perder
        ...
        if(tecla=='w' || tecla=='W' || tecla==72){
            cy--;
            if(cy==0) break; // Se a cabeça da cobra estiver na parede superior 0 jogo acaba
        }   
